...
public class SetCookie extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println(24 - now.getHour());
		System.out.println(60 - now.getMinute());
		System.out.println(60 - now.getSecond());
		
		//오늘 하루가 지날때까지 쿠키가 남아있게 하는법
		try {
			int hour = 24 - now.getHour() -1;
			int minute = 60 - now.getMinute() -1;
			int second = 60 - now.getSecond();
			
			minute = minute + hour * 60;
			second = second + minute * 60;
			
			Cookie c4 = new Cookie("dinner", "라면");		
			c4.setMaxAge(second);
